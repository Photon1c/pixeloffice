...
async function runDbQuery(pool, sql, params) {
    const result = await pool.query(sql, params);
    return result.rows || result;
}
async function ensureMigrationTable(pool, dbType) {
    if (dbType === "postgres") {
...
async function markMigrationApplied(pool, dbType, version) {
    if (dbType === "postgres") {
...
export async function runMigrations() {
    ...
    for (const tableName of Object.keys(dbSchemas)) {
...
async function ensureDefaultUser(pool, dbType) {
    try {
