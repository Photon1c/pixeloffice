 ...
 */
const ASK_INTERROGATIVES = /\b(who|what|why|how|when|where|does|did|is|are|can|could|should|would|do|has|have|will)\b/i;
const AGREE_SIGNALS = /\b(yeah|yes|true|exactly|good point|i agree|totally|absolutely|certainly|right|definitely|for sure)\b/i;
const URGENCY_SIGNALS = /\b(should|need to|must|important|serious|urgent|right now|asap|quick|critical|immediately|have to|gotta|better)\b/i;
const DISAGREE_SIGNALS = /\b(not sure|don't think|disagree|differently|actually|but|however|no|nah|doubt|wouldn't say)\b/i;
export function checkIntentRules(intent, text) {
    switch (intent) {
        case "ask":
