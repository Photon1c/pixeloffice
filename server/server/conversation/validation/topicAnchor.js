 ...
 */
export function checkKeywordOverlap(text, topicKeywords, prevKeywords) {
    const textNorm = normalizeText(text);
    const matchedTopic = topicKeywords.length > 0 && topicKeywords.some((k) => textNorm.includes(k));
    const matchedPrev = prevKeywords.length > 0 && prevKeywords.some((k) => textNorm.includes(k));
    return {
