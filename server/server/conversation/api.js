 ...
 */
import { createCoolerSession, getNextIntent, addUtteranceToHistory, validateUtterance, buildTurnPrompt, getRepairText, sessionToMarkdown, } from "./coolerController";
 ...
 */
export async function runNextTurn(session, generateFn) {
    const speaker = session.participants[session.currentTurn % session.participants.length];
    ...
    if (!utterance) {
        const prevText = session.utterances[session.utterances.length - 1]?.text;
 ...
 */
export async function runSession(options, maxTurns, generateFn) {
