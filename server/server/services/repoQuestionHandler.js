...
export function isRepoQuestion(message) {
...
export function generateReadmeAnswer(fileContent, repoInfo) {
    if (!fileContent) {
...
export function generateStatusAnswer(status, repoInfo) {
    if (!status) {
...
export function generateFilesAnswer(fileList, repoInfo) {
    if (!fileList) {
...
export async function handleRepoQuestion(message, options) {
...
export async function fetchRecentCommits(config) {
    ...
    try {
        const response = await fetch(`https://api.github.com/repos/${config.owner}/${config.repo}/commits?per_page=${config.limit || 5}`, {
            headers: {
                Authorization: `Bearer ${token}`,
                Accept: "application/vnd.github.v3+json",
                "User-Agent": "PixelOffice-RepoQuestionHandler",
...
export async function fetchRecentPRs(config) {
    ...
    try {
        const response = await fetch(`https://api.github.com/repos/${config.owner}/${config.repo}/pulls?state=${config.state || "open"}&per_page=${config.limit || 5}`, {
            headers: {
                Authorization: `Bearer ${token}`,
                Accept: "application/vnd.github.v3+json",
                "User-Agent": "PixelOffice-RepoQuestionHandler",
...
export async function fetchRecentIssues(config) {
    ...
    try {
        const response = await fetch(`https://api.github.com/repos/${config.owner}/${config.repo}/issues?state=${config.state || "open"}&per_page=${config.limit || 5}`, {
            headers: {
                Authorization: `Bearer ${token}`,
                Accept: "application/vnd.github.v3+json",
                "User-Agent": "PixelOffice-RepoQuestionHandler",
...
export function formatAnswerForOffice(result, agentName = "clerk") {
