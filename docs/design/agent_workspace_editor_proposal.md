# Agent Workspace Editor – GUI Upgrade Proposal (OpenCode)

> Pixel Office note (2026-03-31): This proposal currently targets OpenCode’s agent workflows in general. For Pixel Office, we anticipate **future** use of an Agent Workspace Editor for:
> - Agents proposing layout/dash changes via a constrained schema (not arbitrary code edits).
> - Human review + approval of proposals (no fully autonomous merges).
> - Strict separation between **visual/layout config** (editable) and **stigmergic fields** (Review Heat, Task Shadows, social potential), which must remain **derived/read-only**, not directly edited by agents.
> - Initial implementation behind LAB_MODE only.
>
> For now this document is **design only**—no implementation is assumed.
